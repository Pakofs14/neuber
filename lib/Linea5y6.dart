...
void main() => runApp(MaterialApp(home: Linea5y6Page()));

class Linea5y6Page extends StatefulWidget {
  @override
  _Linea5y6PageState createState() => _Linea5y6PageState();
}

class _Linea5y6PageState extends State<Linea5y6Page> {
...
void initState() {
 ...
 fetchAirtableData().then((data) {
  ...
  bool _validateCredentials(String user, String password) {
    return user == 'enrique' && password == '0619';
  }
...
Future<List<Map<String, dynamic>>> fetchAirtableData() async {
 ...
 if (response.statusCode == 200) {
  ...
  if (fields['Linea 3'] == 'Verdadero'&&fields['En Linea'] == 'Verdadero') {
    ...
    String nota = fields['Nota Linea 5 y 6']; // Agrega esta línea
    ...
    result.add({
      ...
      'hora': hora, 
      'Cama L3': fields['Cama L3'], // Agrega el campo Cama L1
      'Huacal L3': fields['Huacal L3'], // Agrega el campo Huacal L1
      'Soldadura L3': fields['Soldadura L3'], // Agrega el campo Interiores L1
      'Interiores L3': fields['Interiores L3'], // Agrega el campo Soldadura L1
...
TextField(
 ...
 onPressed: () {
   ...
   if (username == 'enrique' && password == '0619') {
        ...
        actions: [
          TextButton(
            onPressed: () async {
              ...
              // Actualiza el registro en Airtable
              await updateAirtableRecord(recordId, {'Nota Linea 5 y 6': newNote});
...
void _navigateToDesglosadoSPage(BuildContext context, Map<String, dynamic> data) {
  ...
  if ((data.containsKey('Huacal L3') && data['Huacal L3'] == 'Verdadero') ||
      (data.containsKey('Cama L3') && data['Cama L3'] == 'Verdadero') ||
      (data.containsKey('Interiores L3') && data['Interiores L3'] == 'Verdadero') ||
      (data.containsKey('Patines L3') && data['Patines L3'] == 'Verdadero') ||
      (data.containsKey('Soldadura L3') && data['Soldadura L3'] == 'Verdadero')) {
    Navigator.of(context).push(
      PageRouteBuilder(
        transitionDuration: Duration(milliseconds: 500),
        pageBuilder: (_, __, ___) => DesglosadoSPage(linea: 'L3', proyecto: nombreProyecto), // Pasa nombreProyecto en lugar de 'title'
        ...
        actions: [
          TextButton.icon(
            onPressed: () {
              ...
              // Define recordId before using it
              ...
              if (photoAttachments == null || photoAttachments.isEmpty) {
                ...
                Navigator.of(context).pop();
                _showPhotosDialog(context, photoAttachments, recordId); // Shows Foto Nota
              }
            },
...
void _showFullScreenImage(BuildContext context, List<dynamic> photoAttachments, int initialIndex) {
  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) {
        return Scaffold(
          ...
          body: PhotoViewGallery.builder(
            ...
            builder: (context, index) {
              String? imageUrl = photoAttachments[index] is Map && photoAttachments[index]['url'] != null
                  ? photoAttachments[index]['url']
                  : photoAttachments[index] as String?;

              return PhotoViewGalleryPageOptions(
                imageProvider: imageUrl != null ? NetworkImage(imageUrl) : AssetImage('assets/placeholder.jpg'),
            ...
            pageController: PageController(initialPage: initialIndex), // Navegación entre las imágenes
            enableRotation: true, // Permitir rotación de la imagen
          ),
        );
      },
    ),
...
void _showPhotosDialog(BuildContext context, List<dynamic> photoAttachments, String recordId) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            String? getCurrentImageUrl() {
              if (photoAttachments[currentPhotoIndex] is Map && photoAttachments[currentPhotoIndex]['url'] != null) {
                return photoAttachments[currentPhotoIndex]['url'];
              } else if (photoAttachments[currentPhotoIndex] is String) {
                return photoAttachments[currentPhotoIndex];
              }
              return null;
            }
            ...
            return AlertDialog(
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      _showFullScreenImage(context, photoAttachments, currentPhotoIndex);
                    },
                    child: imageUrl != null
                        ? Image.network(
                            imageUrl,
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text('${currentPhotoIndex + 1} / ${photoAttachments.length}'),
                      IconButton(
                        icon: Icon(Icons.arrow_forward),
                        onPressed: currentPhotoIndex < photoAttachments.length - 1
                            ? () {
                                setState(() {
                                  currentPhotoIndex++;
                                });
              ...
              actions: [
                  ...
                  onPressed: () async {
                    bool? confirmDelete = await showDialog<bool>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: Text('¿Está seguro que desea eliminar esta foto?'),
                        actions: [
                          TextButton(
                    ...
                    if (confirmDelete == true) {
                      await _deletePhotoFromAirtable(
                        photoAttachments[currentPhotoIndex], 
                        recordId,
                        context
                      );

                      // Actualizar la lista de fotos después de la eliminación
                      setState(() {
                        photoAttachments.removeAt(currentPhotoIndex);

                        // Si después de eliminar la foto actual no hay fotos, se cierra el diálogo
                        if (photoAttachments.isEmpty) {
                          Navigator.pop(context);
                        } else {
                          // Si aún quedan fotos, ajustar el índice de la foto actual
                          if (currentPhotoIndex >= photoAttachments.length) {
                            currentPhotoIndex = photoAttachments.length - 1;
                          }
                        }
                      });
                    }
                ...
                TextButton.icon(
                  onPressed: () {
                    _showLoginDialogFoto(recordId);  // Pasamos recordId
                  },
...
Future<void> _deletePhotoFromAirtable(dynamic photoToDelete, String recordId, BuildContext context) async {
  final airtableApiToken = 'patTjJNwpD104BTKG.9352a6a8b38ce585bc3b55de8667ef8e81800fc5cde77e95a95398447a4ca604';
  final airtableBaseId = 'appHba5WGxI7G7VDA';
  final airtableTableName = 'Proyectos';
  ...
  String? photoUrl;
  if (photoToDelete is Map && photoToDelete['url'] != null) {
    photoUrl = photoToDelete['url'];
  } else if (photoToDelete is String) {
    photoUrl = photoToDelete;
  }

  if (photoUrl == null) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('No se pudo identificar la foto para eliminar')),
    );
    return;
  }

  try {
    // Primero, mostrar diálogo de carga
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
    ...
    // Obtener el registro actual para actualizar
    final recordResponse = await http.get(
      Uri.parse('https://api.airtable.com/v0/$airtableBaseId/$airtableTableName/$recordId'),
      headers: {
        'Authorization': 'Bearer $airtableApiToken',
      },
    );
    ...
    if (recordResponse.statusCode == 200) {
      var recordData = json.decode(recordResponse.body);
      var currentPhotos = recordData['fields']['Foto Nota'] ?? [];

      // Filtrar la foto a eliminar
      var updatedPhotos = currentPhotos.where((photo) => 
        photo['url'] != photoUrl
      ).toList();
      ...
      final updateResponse = await http.patch(
        ...
        body: json.encode({
          'fields': {
            'Foto Nota': updatedPhotos
          }
        }),
      );

      // Cerrar diálogos previos
      Navigator.of(context).pop(); // Cerrar diálogo de eliminación
      Navigator.of(context).pop(); // Cerrar diálogo de fotos
      ...
      if (updateResponse.statusCode == 200) {
              ...
              actions: [
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    // Opcional: Recargar las fotos o actualizar la vista
                  },
                ),
              ],
            );
        ...
        throw Exception('Error al actualizar registro en Airtable');
      }
    } else {
      throw Exception('Error al obtener registro de Airtable');
    }
  } catch (e) {
