...
void main() => runApp(MaterialApp(home: LiberacionPage()));

class LiberacionPage extends StatefulWidget {
  @override
  _LiberacionPageState createState() => _LiberacionPageState();
}

class _LiberacionPageState extends State<LiberacionPage> {
...
for (var record in records) {
 ...
 if (fields['En Linea'] == 'Verdadero') {
   int AvanceProyecto1 = fields['Liberados'];
   int RestanteProyecto1 = fields['Restante Liberados'];
...
DateTime fechaEntrega = fields['Fecha de fin'] == null || fields['Fecha de fin'].isEmpty
 ...
 : DateTime.parse(fields['Fecha de fin']);
    ...
    String nombre = 'PT ${fields['Nombre del Proyecto']}';
   String nota = fields['Nota Liberacion']; // Agrega esta línea
...
Widget _buildGraph(List<Map<String, dynamic>> data) {
  ...
  for (var record in data) {
    ...
    if (showAllGraphs || RestanteProyecto1 != 0) { // Cambiado de restanteProyecto1 a RestanteProyecto1
              ...
              if (piezasTerminadas <= restanteSoldadura) {
                ...
                if (avanceProyecto1 >= 0) {
                  ...
                  updateAirtableRecord(data['id'], {
                    'Liberados': avanceProyecto1,
                    'Restante Liberados': restanteProyecto1,
        ...
        actions: [
          TextButton(
            onPressed: () async {
              ...
              // Actualiza el registro en Airtable
              await updateAirtableRecord(recordId, {'Nota Liberacion': newNote});
